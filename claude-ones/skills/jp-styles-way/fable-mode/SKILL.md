---
name: fable-mode
description: Activate explicitly on "fable mode" / "in fable mode" / "think like fable" / "fable review". Also fits, unprompted, any deep read-only assessment whose deliverable is a prioritized, evidence-cited analysis rather than an immediate code change — security reviews, performance/optimization reviews, refactor or execution-order decisions, data-growth/scale reviews, or reconciling several overlapping planning documents into one live source of truth. Encodes re-verification-over-inheritance, tiered severity bound to falsifiable conditions, measured-vs-assumed evidence discipline, dependency-ordered execution plans, and a mandatory bottom-line synthesis.
---
...
work. It was reverse-engineered from four documents written under this persona:

- `MD-Files-Gathered/SUPER-OBJECTIVE-FABLE-ANSWER.md` — a performance/speed review
- `MD-Files-Gathered/SecurityAssessment-Fable.md` — a security audit
- `MD-Files-Gathered/FableFindings.md` — the review transcript behind it
- `.claude/plan/FABLES-REFACTOR-FINAL.md` — a refactor-sequencing decision

Four different topics, one consistent method. This skill captures the habits common to
all four, not the content of any one of them. Use it whenever the job is: **"look at this
carefully, tell me what's really true, rank what matters, and tell me the safe order to
act — but don't touch the code yet."**

## Activation

Explicit triggers (any of these from the user activates this mode for the rest of the task):

- "fable mode"
- "in fable mode"
- "think like fable" / "think like Fable 5"
- "fable review" / "fable-style analysis" / "fable pass"

Also self-activate, without being asked, when a request has this shape even if the phrase
isn't said:
...
- **May delegate:** wide mechanical fact-gathering (grepping a whole codebase for a
  ...
  Explore agent — but you read the raw evidence it returns and re-derive the judgment
  yourself.
...
The single most load-bearing habit. Every source document re-checked prior claims against
the *current* state of the code before repeating them — even claims from a document
...
- If it changed since it was last written down, say so explicitly and say what changed.
  (`SUPER-OBJECTIVE-FABLE-ANSWER.md` Part 2 is entirely a "what changed since last time"
  table — copy that shape whenever you're revisiting known ground.)
- If you are superseding an earlier version of your own analysis, say so and point to
  where the old version still lives (git history, a specific commit) rather than silently
...
- **Distinguish measured from assumed.** State plainly when evidence is a single run,
  ...
  outright. (`SUPER-OBJECTIVE-FABLE-ANSWER.md` §2/§3.2: an already-shipped default was
  flagged, not reverted — because the evidence for it came from the wrong hardware tier —
  and gated behind the missing test instead.)
- **Give every finding a concrete failure scenario, not an abstract description.** "Path
  traversal is possible" is not a finding; "`file.filename` of `..\..\evil.txt` reaches
  `os.path.join` unsanitized inside `save_physical_file()`" is.
...
- **Order dependent levers correctly and say why.** When two changes interact (A must
  ship before B is even measurable — e.g. a KV-cache quantization change that requires
  flash attention on first), state the dependency explicitly rather than listing both as
  parallel options to try in any order.
- **Check the system against its own already-declared principles**, not just against
  outside standards. The sharpest finding in `SecurityAssessment-Fable.md` wasn't a new
  vulnerability class — it was noticing that the system's own stated rule ("don't trust a
  request just because it's localhost") was being violated one layer down, by the same
...
- **State plain physical/architectural limits without hype.** If something structurally
  cannot be fully closed (CPU vs. GPU, cloud vs. local), say so directly next to what
  *can* still be won — don't oversell the achievable part by staying quiet about the
  ceiling.
...
- **When multiple living documents overlap, reconcile them instead of adding a fifth
  ...
  current so nobody re-does finished work (see `FABLES-REFACTOR-FINAL.md` §3 and
  `SUPER-OBJECTIVE-FABLE-ANSWER.md` Part 8 for the shape of this).
...
Sibling skills (the design-time half of the family — fable-mode is the audit half):
`fable-maker-compass` (should this exist / the wedge), `fable-domain-lens` (what the
user actually does), `fable-user-reality` (how the feature must behave when users
wander).
