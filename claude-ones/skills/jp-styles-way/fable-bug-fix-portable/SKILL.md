---
name: fable-bug-fix-portable
description: Portable, product-agnostic version of fable-bug-fix - copy this folder into any new project's .claude/skills/. In this repo, prefer fable-bug-fix; activate this one explicitly via "portable bug fix" or in a non-PRIVATE codebase. Encodes literal-repro-first, "disappeared is a hypothesis not a diagnosis", the fact census (enumerate every copy of the fact before touching any one of them), silent-write-paths-are-guilty-first, the sibling-writer diff, re-read-the-file-not-the-narration on round 2+, and an honest confidence ladder where "typechecks clean" is never "verified". Fully self-contained - needs no companion documents.
---

# Fable Bug-Fix Mode (Portable)
...
find the real bug and to be honest about how verified the fix actually is. Every example
below is described self-containedly; this file needs no companion documents.

## Origin — the worked example (compressed, self-contained)

An editor app where a record's working draft lived in **three places**: the database row,
a debounced localStorage autosave snapshot, and the in-memory list/array the UI reopens
records from.

| Round | Claimed fix | Actual root cause found later |
|---|---|---|
| 1 | Reopen path now overlays a newer local snapshot over the raw DB row | Reopen read straight from the stale DB object, ignoring a newer localStorage snapshot tagged for that exact record |
| 2 | New durable debounced DB autosave; cache demoted to crash-recovery | The localStorage snapshot was **one slot per parent** (client), not per record — opening a second draft for the same parent silently evicted the first draft's unsaved edit before it ever reached the DB |
| 3 | The round-2 autosave's `.then()` now also updates the in-memory list | The round-2 DB write was **correct** — but nothing told the in-memory list it happened, so reopening the record repainted the pre-write copy over the screen |
...
Round 2 was one cache slot vs. N records wanting it. Round 3 was DB vs. the in-memory
...
re-reading the literal current file, and (b) for the new write the previous fix had
introduced, grepping *every other call site* that writes the same record type and diffing
what they update afterward. Three of four already updated the in-memory list. One didn't.
That diff was the bug.
...
- "portable bug fix"
- "fable bug fix" / "bug-fix mode" / "fable bug-fix mode" (when no app-specific sibling
  skill exists in the project)

Also self-activate, without being asked, when a request has this shape (in a project
without an app-specific sibling):
...
- Do not generalize prematurely. "Switch to a different record **under the same parent**,
  then come back" is a different code path from "switch to another parent" — the specific
...
attached, and both failed live testing. "Typechecks clean" is not "verified", and
blending them into one "done" burns the user's trust and their time.

If rung 4 is unreachable in your environment — the only database is real production data,
there's no scratch workspace, the app is credential-locked — **say so explicitly, do not
fake it, and do not quietly skip it.** Hand back numbered repro steps in the user's own
scenario (including "restart the app first so the fix actually loads"), and state plainly
that the fix is code-traced, not click-verified. That honesty is what lets the user's
live test function as the real rung 4 instead of a surprise round N+1.
...
```markdown
Fact under investigation: [e.g. "a record's working-draft text"]
...
- Treating navigation-heavy repros (switch, go elsewhere, close app, return) as edge
  cases.
...
Sibling skills (copy as a set if useful — this is the debugging half, the others are
design-time and audit-time): `fable-mode-portable` (deep read-only assessment),
`fable-user-reality-portable` (how features must behave when users wander — its
wandering law is this skill's Phase 0 stance), `fable-domain-lens-portable` (what the
user actually does), `fable-maker-compass-portable` (should this exist at all).
